...
export async function POST(req: NextRequest) {
    try {
        ...
        const articleContent = data(".article-wrap");
        ...
        articleContent.find("p").each((_, el) => {
            const paragraph = data(el);

            const text = paragraph.text().trim();

            if (/^Read more/i.test(text) || /^Download the Yahoo/i.test(text)) {
                paragraph.remove();
            }
        });
        ...
        const rawDate =
            data("time.byline-attr-meta-time").attr("datetime") || "";
        let date = "";
